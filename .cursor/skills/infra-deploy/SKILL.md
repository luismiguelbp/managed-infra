---
name: infra-deploy
description: Deploy and operate the managed-infra Linux fleet via bin/ scripts and Ansible. Use when running infra-bootstrap, infra-deploy-edge-stack, infra-backup-edge-stack, infra-restore-edge-stack, infra-reboot, fleet status checks, MANAGED_INFRA_CONFIG_SRC, Ansible playbooks, or provisioning Docker Compose edge stack services on remote hosts (services per host profile in inventory).
---

# managed-infra fleet deploy

Infrastructure-as-code for a small Linux server fleet (Debian, Raspberry Pi OS, and similar). Run all commands from the **managed-infra repo root** on your control machine (Mac).
...
- SSH key access; passwordless sudo on each host (`docs/ansible.md`)
...
| Edge stack only (Compose services per host profile) | `./bin/infra-deploy-edge-stack` |
...
2. `./bin/infra-ping` — verify SSH (add `--limit edge-node-1` for one host)
3. Run the chosen script (add `--limit <host>` for staged rollout)
4. Verify: `./bin/infra-docker-status`

### Full vs edge-only

- **New host or infra change** → `infra-bootstrap`
...
Add `--limit edge-node-1` for a single host.
...
Requires `--limit` with exactly one host. Default source folder matches the limited host; override with `edge_stack_restore_source`. Restores `data/` and service dumps under `dumps/` only unless `-e edge_stack_restore_env=true` (reconciles `COMPOSE_FILE` and `COMPOSE_PROJECT_NAME` for the target).

## Credentials (manual, not in Ansible vars)

Per-service setup on each host (`.env`, files under `docker/data/`). See `docker/README.md#credentials`.
